...
const FranchiseDashboard = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Franchise Dashboard</h1>
        <p className="text-sm text-slate-500 mt-0.5">
          {isAllBranches ? 'Aggregate across all branches' : `Viewing: ${branches.find(b => b.id === currentBranch)?.name ?? 'Branch'}`}
        </p>
      </div>
